...
export default function Admin() {
        ...
        .map(group => ({
          ...
          options: safeArray(group.options)
            .map((option, optionIndex) => ({
              id: String(option?.id || `${group.id}-option-${optionIndex}`).trim(),
  ...
  return <div className="admin-page" dir="rtl">
    ...
    <main className="admin-main admin-horizontal-safe">
      <style>{`
        .admin-horizontal-safe, .admin-horizontal-safe * { box-sizing: border-box; }
        .admin-horizontal-safe { min-width: 0; max-width: 100%; overflow-x: hidden; }
        .admin-horizontal-safe .admin-card,
        .admin-horizontal-safe .admin-form,
        .admin-horizontal-safe .settings-block,
        .admin-horizontal-safe .dashboard-grid,
        .admin-horizontal-safe .section-header,
        .admin-horizontal-safe .form-grid,
        ...
        @media (max-width: 900px) {
          ...
          .admin-horizontal-safe .admin-wide-row-scroll > * { min-width: 650px; }
        }
        @media (max-width: 600px) {
          .admin-horizontal-safe .admin-table { min-width: 720px; }
          .admin-horizontal-safe .admin-wide-row-scroll > * { min-width: 620px; }
        }
      `}</style>
                  ...
                  <div style={{ marginTop: 12 }}>
                    ...
                    <div style={{ display: "grid", gap: 8, marginTop: 10 }}>
                      {group.options.map((option, optionIndex) => (
                        <div key={option.id || `${group.id}-${optionIndex}`} className="admin-wide-row-scroll"><div style={{ display: "grid", gridTemplateColumns: "minmax(220px,1fr) minmax(220px,1fr) 150px auto", gap: 8, alignItems: "end", minWidth: 700 }}>
                          <label>
                            <span>القيمة</span>
      ...
      {tab === "reports" && <Reports orders={orders} products={products} users={users} categories={categories} banners={banners} announcementBars={announcementBars} popupAds={popupAds} activityLogs={activityLogs} categoryVisits={categoryVisits} favorites={favorites} support={support} />}
...
function Reports({orders,products,users,categories,banners,announcementBars,popupAds,activityLogs,categoryVisits,favorites,support}) {
  ...
  const reportDate = (value) => {
    ...
    if (value?.nanoseconds != null && value?.seconds != null) return new Date(Number(value.seconds) * 1000 + Math.floor(Number(value.nanoseconds) / 1000000));
    const d = value instanceof Date ? value : new Date(value);
    return Number.isNaN(d.getTime()) ? null : d;
  };
  const activityDate = (x) => reportDate(x?.visitedAt || x?.createdAt || x?.updatedAt || x?.timestamp || x?.date);
  const customerKey = (x) => String(x?.userId || x?.uid || x?.customerId || x?.userUid || x?.visitorId || x?.guestId || x?.visitorUid || x?.customerEmail || x?.email || x?.phone || x?.customerPhone || "").trim();
  const customerName = (x) => x?.name || x?.customerName || x?.displayName || x?.email || x?.phone || "زائر";
  const categoryName = (visit) => {
    const id = String(visit?.categoryId || visit?.category || visit?.categoryID || "").trim();
    const direct = visit?.categoryName || visit?.categoryTitle || visit?.name;
    if (direct) return String(direct);
    const found = categories.find(c => String(c.id) === id);
    return found?.name || id || "غير محدد";
  };
  ...
  const sameCustomer = (a,b) => {
    const ak=customerKey(a), bk=customerKey(b);
    return ak && bk && ak===bk;
  };
  const userMatches = (item,user) => sameCustomer(item,user);

  const visitsWithDate = safeArray(categoryVisits).map(v=>({...v,__date:activityDate(v)})).filter(v=>v.__date);
  const weeklyMap = {};
  visitsWithDate.forEach(v=>{
    const start=weekStart(v.__date); const key=start.toISOString().slice(0,10);
    if(!weeklyMap[key]) weeklyMap[key]={key,start,visits:0,customers:new Set(),categories:new Set()};
    weeklyMap[key].visits++;
    const ck=customerKey(v); if(ck) weeklyMap[key].customers.add(ck);
    weeklyMap[key].categories.add(categoryName(v));
  });
  const weeklyReports = Object.values(weeklyMap).sort((a,b)=>b.start-a.start).map(w=>({...w,label:weekLabel(w.start),uniqueCustomers:w.customers.size,categoryCount:w.categories.size}));

  const customerMap = {};
  safeArray(users).forEach(u=>{
    const key=customerKey(u); if(!key) return;
    customerMap[key]={key,name:customerName(u),email:u.email||"",phone:u.phone||"",visits:0,categories:new Set(),weeks:new Set(),orders:0,spent:0,favorites:0,support:0,activity:0,lastActivity:null};
  });
  visitsWithDate.forEach(v=>{
    const key=customerKey(v); if(!key) return;
    if(!customerMap[key]) customerMap[key]={key,name:customerName(v),email:v.email||v.customerEmail||"",phone:v.phone||v.customerPhone||"",visits:0,categories:new Set(),weeks:new Set(),orders:0,spent:0,favorites:0,support:0,activity:0,lastActivity:null};
    const c=customerMap[key]; c.visits++; c.categories.add(categoryName(v)); c.weeks.add(weekLabel(v.__date)); c.lastActivity=!c.lastActivity||v.__date>c.lastActivity?v.__date:c.lastActivity;
  });
  safeArray(orders).forEach(o=>{
    const key=customerKey(o); if(!key) return;
    if(!customerMap[key]) customerMap[key]={key,name:customerName(o),email:o.email||"",phone:o.phone||"",visits:0,categories:new Set(),weeks:new Set(),orders:0,spent:0,favorites:0,support:0,activity:0,lastActivity:null};
    const c=customerMap[key]; c.orders++; c.spent+=asNumber(o.total??o.finalTotal); const d=reportDate(o.createdAt||o.updatedAt); if(d) c.lastActivity=!c.lastActivity||d>c.lastActivity?d:c.lastActivity;
  });
  safeArray(favorites).forEach(f=>{const key=customerKey(f); if(key&&customerMap[key]) customerMap[key].favorites++;});
  safeArray(support).forEach(m=>{const key=customerKey(m); if(key&&customerMap[key]){customerMap[key].support++; const d=reportDate(m.createdAt||m.updatedAt); if(d) customerMap[key].lastActivity=!customerMap[key].lastActivity||d>customerMap[key].lastActivity?d:customerMap[key].lastActivity;}});
  safeArray(activityLogs).forEach(a=>{const key=customerKey(a); if(key&&customerMap[key]){customerMap[key].activity++; const d=reportDate(a.createdAt||a.updatedAt); if(d) customerMap[key].lastActivity=!customerMap[key].lastActivity||d>customerMap[key].lastActivity?d:customerMap[key].lastActivity;}});
  const customerReports=Object.values(customerMap).filter(c=>c.visits||c.orders||c.favorites||c.support||c.activity).sort((a,b)=>(b.visits+b.orders+b.activity)-(a.visits+a.orders+a.activity));

  const categoryMap={};
  safeArray(categories).forEach(c=>categoryMap[c.id]={id:c.id,name:c.name||"قسم",visits:0,customers:new Set(),orders:0,sales:0,products:products.filter(p=>String(p.categoryId||p.category)===String(c.id)).length});
  visitsWithDate.forEach(v=>{const id=String(v.categoryId||v.category||""); const name=categoryName(v); const key=id||`name:${name}`; if(!categoryMap[key]) categoryMap[key]={id:key,name,visits:0,customers:new Set(),orders:0,sales:0,products:products.filter(p=>String(p.categoryId||p.category)===id).length}; categoryMap[key].visits++; const ck=customerKey(v); if(ck) categoryMap[key].customers.add(ck);});
  safeArray(orders).forEach(o=>safeArray(o.items||o.products).forEach(it=>{const id=String(it.categoryId||it.category||o.categoryId||o.category||""); const cat=categoryMap[id]; if(cat){cat.orders+=asNumber(it.quantity,1); cat.sales+=asNumber(it.price)*asNumber(it.quantity,1);}}));
  const categoryReports=Object.values(categoryMap).sort((a,b)=>b.visits-a.visits).map(c=>({...c,uniqueCustomers:c.customers.size}));
  ...
  const exportCategories=()=>downloadExcel("categories-report","تقرير الأقسام",["القسم","الزيارات","العملاء","الطلبات","المبيعات","المنتجات"],categoryReports.map(c=>[c.name,c.visits,c.uniqueCustomers,c.orders,c.sales,c.products]));
  ...
  const exportUsers=()=>downloadExcel("users-report","تقرير العملاء",["الاسم","البريد","الهاتف","الزيارات","الأقسام","الطلبات","المبيعات","المفضلة","الدعم","النشاط","آخر نشاط"],customerReports.map(c=>[c.name,c.email,c.phone,c.visits,[...c.categories].join("، "),c.orders,c.spent,c.favorites,c.support,c.activity,c.lastActivity?c.lastActivity.toLocaleString("ar-EG"):"—"]));
  ...
  const exportVisits=()=>downloadExcel("weekly-visits-report","تقرير الزيارات الأسبوعي",["الأسبوع","عدد الزيارات","العملاء الفريدون","الأقسام التي تمت زيارتها"],weeklyReports.map(w=>[w.label,w.visits,w.uniqueCustomers,w.categoryCount]));
  const exportCustomerActivity=()=>downloadExcel("customer-activity-report","تقرير نشاط العملاء",["العميل","البريد","الهاتف","الزيارات","الأقسام التي زارها","الأسابيع","الطلبات","المبيعات","المفضلة","رسائل الدعم","سجل النشاط","آخر نشاط"],customerReports.map(c=>[c.name,c.email,c.phone,c.visits,[...c.categories].join("، "),[...c.weeks].join(" | "),c.orders,c.spent,c.favorites,c.support,c.activity,c.lastActivity?c.lastActivity.toLocaleString("ar-EG"):"—"]));

  return <section className="admin-card">
    <div className="section-header"><div><h2>📊 التقارير</h2><p>تقارير أداء مفصلة للمنتجات والأقسام والزيارات والعملاء والنشاط، مع تصدير Excel.</p></div></div>
    <div className="stats-grid">
      <div className="report-box"><span>إجمالي المنتجات</span><strong>{products.length}</strong></div><div className="report-box"><span>إجمالي الأقسام</span><strong>{categories.length}</strong></div><div className="report-box"><span>طلبات مكتملة</span><strong>{delivered.length}</strong></div><div className="report-box"><span>طلبات ملغاة</span><strong>{cancelled.length}</strong></div><div className="report-box"><span>العملاء</span><strong>{users.length}</strong></div><div className="report-box"><span>زيارات الأقسام</span><strong>{visitsWithDate.length}</strong></div><div className="report-box"><span>عملاء زاروا الأقسام</span><strong>{new Set(visitsWithDate.map(customerKey).filter(Boolean)).size}</strong></div><div className="report-box"><span>المبيعات</span><strong>{money(sales.reduce((s,o)=>s+asNumber(o.total??o.finalTotal),0))}</strong></div>
    </div>
    <div className="settings-block"><h3>📥 تصدير تقارير Excel</h3><div className="form-actions report-actions"><button type="button" className="save-btn" onClick={exportProducts}>📦 المنتجات</button><button type="button" className="save-btn" onClick={exportCategories}>🗂️ الأقسام بالتفصيل</button><button type="button" className="save-btn" onClick={exportOrders}>🛒 الطلبات</button><button type="button" className="save-btn" onClick={exportUsers}>👥 العملاء والنشاط</button><button type="button" className="save-btn" onClick={exportSales}>💰 المبيعات</button><button type="button" className="save-btn" onClick={exportVisits}>📈 الزيارات أسبوعيًا</button><button type="button" className="save-btn" onClick={exportCustomerActivity}>👤 نشاط العملاء الكامل</button><button type="button" className="save-btn" onClick={exportBanners}>🖼️ البانرات</button><button type="button" className="save-btn" onClick={exportBars}>📜 أشرطة الإعلانات</button><button type="button" className="save-btn" onClick={exportPopups}>🔔 الإعلانات المنبثقة</button><button type="button" className="save-btn" onClick={exportActivity}>🧾 سجل النشاط</button></div></div>

    <div className="settings-block"><h3>📈 الزيارات والعملاء أسبوعيًا</h3><div className="table-scroll"><table className="admin-table"><thead><tr><th>الأسبوع</th><th>عدد الزيارات</th><th>عملاء فريدون</th><th>أقسام تمت زيارتها</th></tr></thead><tbody>{weeklyReports.length?weeklyReports.map(w=><tr key={w.key}><td>{w.label}</td><td>{w.visits.toLocaleString("ar-EG")}</td><td>{w.uniqueCustomers.toLocaleString("ar-EG")}</td><td>{w.categoryCount.toLocaleString("ar-EG")}</td></tr>):<tr><td colSpan="4">لا توجد بيانات زيارات حتى الآن.</td></tr>}</tbody></table></div></div>

    <div className="settings-block"><h3>🗂️ تقرير مفصل لكل قسم</h3><p>عدد الزيارات، العملاء الفريدين، الطلبات، المبيعات وعدد المنتجات داخل كل قسم.</p><div className="table-scroll"><table className="admin-table"><thead><tr><th>القسم</th><th>المنتجات</th><th>الزيارات</th><th>عملاء فريدون</th><th>الطلبات</th><th>المبيعات</th></tr></thead><tbody>{categoryReports.map(c=><tr key={c.id}><td>{c.name}</td><td>{c.products}</td><td>{c.visits}</td><td>{c.uniqueCustomers}</td><td>{c.orders}</td><td>{money(c.sales)}</td></tr>)}</tbody></table></div></div>

    <div className="settings-block"><h3>👥 تفاصيل نشاط العملاء</h3><p>كل عميل: الأقسام التي زارها، عدد الزيارات، الأسابيع التي ظهر فيها، الطلبات، المبيعات، المفضلة، رسائل الدعم وسجل النشاط وآخر نشاط.</p><div className="table-scroll"><table className="admin-table"><thead><tr><th>العميل</th><th>البريد</th><th>الهاتف</th><th>الزيارات</th><th>الأقسام التي زارها</th><th>الأسابيع</th><th>الطلبات</th><th>المبيعات</th><th>المفضلة</th><th>الدعم</th><th>النشاط</th><th>آخر نشاط</th></tr></thead><tbody>{customerReports.length?customerReports.map(c=><tr key={c.key}><td>{c.name}</td><td>{c.email||"—"}</td><td>{c.phone||"—"}</td><td>{c.visits}</td><td>{[...c.categories].join("، ")||"—"}</td><td>{[...c.weeks].join(" | ")||"—"}</td><td>{c.orders}</td><td>{money(c.spent)}</td><td>{c.favorites}</td><td>{c.support}</td><td>{c.activity}</td><td>{c.lastActivity?c.lastActivity.toLocaleString("ar-EG"):"—"}</td></tr>):<tr><td colSpan="12">لا توجد بيانات نشاط عملاء كافية حتى الآن.</td></tr>}</tbody></table></div></div>
  </section>;
}
