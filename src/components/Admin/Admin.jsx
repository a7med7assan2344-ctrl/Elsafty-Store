...
function Field({ spec, value, onChange, categories }) {
  ...
  if (type === "file") return <label><span>{label}</span><input type="file" name={name} accept="image/jpeg,image/jpg,image/png,image/webp" onChange={onChange} /></label>;
...
const normalizeVariant = (variant = {}, index = 0) => {
  ...
  return {
    id: variant.id || `variant-${Date.now()}-${index}-${Math.random().toString(36).slice(2, 7)}`,
...
export default function Admin() {
  ...
  return <div className="admin-page" dir="rtl">
    ...
    <main className="admin-main">
      ...
      {userDetails && <Modal title="بيانات العميل" onClose={()=>setUserDetails(null)}><div className="profile-detail"><div className="large-avatar">{String(userDetails.name||"ع").slice(0,1)}</div><h3>{userDetails.name||"عميل"}</h3><p dir="ltr">{userDetails.email||"—"}</p><p dir="ltr">{userDetails.phone||"—"}</p><span className={userDetails.blocked?"status-inactive":"status-active"}>{userDetails.blocked?"🔴 محظور":"🟢 نشط"}</span></div><div className="form-actions"><button type="button" className="cancel-btn" onClick={()=>toggleUserBlock(userDetails)}>{userDetails.blocked?"✅ إلغاء الحظر":"🚫 حظر العميل"}</button></div></Modal>}
    </main>
  </div>;
}
